...
<div class="max-w-3xl mx-auto py-10 px-4">
    ...
    <h1 class="text-3xl font-bold mb-6 text-gray-800">Formulario Dinámico</h1>
    ...
    <form action="{{ route('answers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                            ...
                            @switch($question->type)
                                ...
                                @case('text')
                                    <input type="text" 
                                           name="{{ $fieldName }}" 
                                           value="{{ $oldValue }}"
                                           class="border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 w-full">
                                    @break
                                ...
                                @case('textarea')
                                    <textarea name="{{ $fieldName }}" 
                                              rows="3"
                                              class="border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 w-full">{{ $oldValue }}</textarea>
                                    @break
                                ...
                                @case('date')
                                    <input type="date" 
                                           name="{{ $fieldName }}" 
                                           value="{{ $oldValue }}"
                                           {{-- Usamos null coalescing (??) por si no definiste opciones --}}
                                           min="{{ $question->options['min_date'] ?? '' }}"
                                           max="{{ $question->options['max_date'] ?? '' }}"
                                           class="border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 w-full">
                                    @break
        ...
        <div class="flex justify-end">
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded shadow-lg transition duration-150">
                Enviar Respuestas
            </button>
        </div>
    </form>
